...
for n in agents:
    df = pd.read_csv(f"results_with_loss_min_rtt_{n}_agents.csv")
    ...
    print(f"{n} Agents → Avg. Loss: {avg_loss:.2f}")
...
for n in agents:
    df = pd.read_csv(f"results_with_loss_min_load_{n}_agents.csv")
